...
// ✅ Minimal interface for TypeScript
interface ImageType {
  id: string;
  url: string;
  createdAt: string;
...
export default function GalleryDashboardPage() {
  ...
  // ✅ Typed SWR
  const { data: images = [], mutate } = useSWR<ImageType[]>(
    "/api/user/images",
    fetcher,
  ...
  return (
    <div style={{ padding: 20 }}>
      <Stack>
        <Text fw={600}>
          Welcome, {session?.user?.name} ({isAdmin ? "Admin" : "User"})
        </Text>

        {!isAdmin && (
          <Alert icon={<IconAlertCircle size={16} />} title="Free Upload Limit" color="red">
            Only {MAX_FREE_IMAGES} images allowed.
          </Alert>
        )}

        <Group justify="space-between">
          <Title order={3}>Gallery</Title>
          <ActionIcon size="lg" color="blue" variant="filled" onClick={() => setModalOpened(true)}>
            <IconPlus size={20} />
          </ActionIcon>
        </Group>
          ...
          <Group gap="md" wrap="wrap">
            {images.map((img: ImageType) => (
              <Card key={img.id} shadow="sm" w={240}>
                ...
                {/* ✅ Safe check for taggedUsers */}
                {(img.taggedUsers ?? []).length > 0 && (
                  <Box mt="xs">
                    <Text size="xs" fw={500}>Tagged:</Text>
                    <Group gap="xs" mt={4}>
                      {img.taggedUsers?.map(u => (
                        <Badge key={u.id} color="blue">{u.name}</Badge>
                      ))}
                    </Group>
                  </Box>
                )}

                <Button fullWidth mt="xs" color="red" size="xs" onClick={() => handleDelete(img.id)}>
                  Delete
                </Button>
              </Card>
            ))}
        ...
        <Modal
          ...
          styles={{
            header: { backgroundColor: "#f8f5f2", borderBottom: "1px solid #eee" },
            title: { fontWeight: 600, fontSize: "18px" },
            body: { paddingTop: 20, paddingBottom: 25 },
          }}
        >
          <Stack gap="md">
            <Box
            ...
            <Button
              ...
              style={{ backgroundColor: "#5c4033", color: "white", fontWeight: 600 }}
              radius="md"
            >
              Upload Images
            </Button>
